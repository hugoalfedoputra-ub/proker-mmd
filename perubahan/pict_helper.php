...
function ResizeGambar($filename, $path, $dimensi)
{
    ...
    // Check file type; png image doesn't have EXIF data
    $imgdata = null;
    if ($file_type === 'jpg' || $file_type === 'jpeg') {
        $imgdata = @exif_read_data($source_path, 'IFD0');
    }

    // Load CI instance and image library
    $ci = &get_instance();
    ...
    // Adjust orientation for JPEG images
    if ($imgdata !== false && isset($imgdata['Orientation'])) {
        $config = [
            'image_library' => 'gd2',
        ...
        switch ($imgdata['Orientation']) {
            ...
            case 2: // Top, right side (Mirror horizontal)
                if (!flip_image($target_path, 'horizontal')) {
                    return false;
                }
                break;
            ...
            case 4: // Bottom, left side (Mirror vertical)
                if (!flip_image($target_path, 'vertical')) {
                    return false;
                }
                break;
            ...
            case 8: // Left side, bottom (Rotate 270 CW)
                ...
                break;
            default:
                // Default action for undefined orientations
                $config['rotation_angle'] = '0';
                break;
        }

        if (isset($config['rotation_angle']) && $config['rotation_angle'] != '0') {
...
function flip_image($image_path, $mode)
{
    $source = imagecreatefromjpeg($image_path);
    if ($source === false) {
        session_error('Failed to create image from source.');
        return false;
    }
    ...
    if ($mode == 'horizontal') {
        ...
        session_error('Invalid flip mode specified.');
        return false;
    }

    if (!imagejpeg($flipped, $image_path)) {
        session_error('Failed to save flipped image.');
        imagedestroy($source);
        imagedestroy($flipped);
        return false;
    }
...
function ResizeGambarOld($filename, $path, $dimensi)
{
    $source_path = $filename;
    $target_path = $path;
    ...
    // Read: https://stackoverflow.com/questions/57771795/how-to-fix-exif-orientation-data-in-jpg-files
    switch ($imgdata['Orientation']) {
        case 1: // Top, left side (Horizontal / normal)
            $config['rotation_angle'] = '0';
            break;
